...
function Axis:AddProvider(provider: Provider): Provider
	...
	elseif self._Providers[provider.Name] ~= nil then
		error("Provider \"" .. provider.Name .. "\" already exists", 2)
	end
...
function Axis:Start()
	...
	local prepareDone = 0
	for _ in pairs(self._Providers) do
		numProviders += 1
	end
	
	-- Call all AxisPrepare methods:
	local thread = coroutine.running()
	for _,provider: Provider in pairs(self._Providers) do
		if typeof(provider.AxisPrepare) == "function" then
			task.spawn(function()
				self:_RunExtensions("BeforePrepare", provider)
				debug.setmemorycategory(provider.Name .. "Prepare")
